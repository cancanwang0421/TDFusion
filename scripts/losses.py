...
class fusion_loss_text(nn.Module):
    def __init__(self):
        super(fusion_loss_text, self).__init__()
...
class fusion_prompt_loss(nn.Module):
    def __init__(self):
        ...
        self.fusion_loss_text = fusion_loss_text()
    ...
    def forward(self, image_A, image_B, image_fused, task):
        ...
        for idx, task_type in enumerate(task):
            ...
            if task_type == "low_light":
                loss, ssim_loss, max_loss, color_loss, grad_loss = self.fusion_loss_text(img_A, img_B, img_fused,
